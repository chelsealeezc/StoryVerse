...
export function ResonancePage({
  ...
  const dimensions = [
    ...
    {
      key: "stage" as const,
      title: state.language === "zh" ? "人生阶段" : "Life stage",
      icon: "◷",
      similar:
        state.language === "zh"
          ? "看看相近年龄和阶段的人如何走过"
          : "See how people of a similar age or stage moved through it",
      different: state.language === "zh" ? "听见另一个人生阶段的声音" : "Hear a voice from another life stage",
    },
    {
      key: "theme" as const,
  ...
  return (
    <main className={`resonance-page ${themeMode === "night" ? "theme-night" : ""}`}>
      <header className="topbar app-shell-header">
        ...
        <div className="topbar-actions">
          <button className="button button-ghost mini" onClick={onBack}>
            <ArrowLeft size={16} /> {text.backToTraits}
          </button>
      ...
      <div className="resonance-action">
        <PrimaryButton onClick={onContinue}>{text.findStories}</PrimaryButton>
        <small>
