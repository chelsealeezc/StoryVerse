...
import { getScene, tourUi } from "./tour-steps";
import type { Placement, TourSceneId } from "./tour-steps";
  ...
  useLayoutEffect(() => {
    ...
    const attempt = (tries: number) => {
      ...
      if (offscreen) {
        ...
        timer = window.setTimeout(() => { if (!cancelled) measure(); }, 420);
      }
      measure();
    };
  ...
  if (centered) {
    ...
    if (placement === "top" || placement === "bottom") {
      ...
      top = placement === "top"
        ...
        : (below + cardH <= window.innerHeight - EDGE ? below : Math.max(EDGE, above));
      left = r.left + r.width / 2 - CARD_W / 2;
    } else {
      const leftPos = r.left - GAP - CARD_W;
      const rightPos = r.left + r.width + GAP;
      left = placement === "left"
        ? (leftPos >= EDGE ? leftPos : rightPos)
        : (rightPos + CARD_W <= window.innerWidth - EDGE ? rightPos : Math.max(EDGE, leftPos));
    ...
    <div className="tour-layer" role="dialog" aria-modal="true" aria-label={copy.title}>
      ...
      <div className="tour-card" ref={cardRef} style={cardStyle}>
        ...
        <div className="tour-actions">
          {index > 0 && <button className="tour-back" onClick={back}>{ui.back}</button>}
          <button className="tour-next" onClick={next}>{last ? finishLabel : ui.next}</button>
        </div>
      </div>
    </div>
  );
