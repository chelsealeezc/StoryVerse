...
import { validateDraft, validatePassword, type StoryDraftInput } from "../_shared/validation.ts";
...
async function dashboard(admin: ReturnType<typeof adminClient>) {
  const [reviews, users, stories, tasks, feedback, types, configs, imports, failures] = await Promise.all([
    admin
      .from("review_cases")
      .select(
  ...
  for (const result of [reviews, users, stories, tasks, feedback, types, configs, imports, failures])
...
serve(async (request) => {
  ...
  if (action === "review-decide") {
    const reviewId = String(input.reviewId ?? "");
    const decision = input.decision === "approved" ? "approved" : "needs_edit";
  ...
  if (action === "account-status") {
    ...
    const status = input.status === "active" ? "active" : "suspended";
  ...
  if (action === "seed-update") {
    ...
    const draft = normalizeDraftShape(
      ...
      } as StoryDraftInput) as StoryDraftInput & Record<string, unknown>,
    );
