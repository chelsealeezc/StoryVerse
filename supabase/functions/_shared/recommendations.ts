...
export async function currentRecommendations(client: SupabaseClient, userId: string, limit: number) {
  ...
  const { data: seedStories, error: seedError } = await client
    .from("stories")
    .select("*,story_type:story_types!stories_final_type_id_fkey(color),generated_images(public_url,status,created_at)")
    .eq("status", "published")
    .eq("source_kind", "seed")
    .order("published_at", { ascending: false })
    .limit(safeLimit);
  if (seedError) throw seedError;
  return (seedStories ?? []).map((story) => ({ story: storyPayload(story), reason: "StoryVerse 精选故事" }));
}
