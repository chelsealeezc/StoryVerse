...
serve(async (request) => {
  ...
  try {
    ...
    if (!response.ok) return json(request, { places: [], degraded: true });
    payload = await response.json();
  } catch {
    // The browser merges this response with src/data/cities.ts, so an empty
    // successful response preserves local search and manual input during outages.
    return json(request, { places: [], degraded: true });
  }
  ...
  return json(request, { places, degraded: false });
});
