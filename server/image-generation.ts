...
export const IMAGE_STYLES = ["crayon", "zine", "minimal-realistic", "retro-collage"] as const;
...
const STYLE_PROMPTS: Record<ImageStyle, string> = {
  crayon: `媒介锁定：高品质写实黏土定格动画 3D 作品（realistic claymation 3D artwork），阿德曼动画（Aardman Animation）风格的荒诞黏土定格世界，呈现真实电影级黏土动画质感，像一帧来自高预算黏土动画电影的画面 —— 不是普通卡通或二维作品。
严格保持构图、镜头角度、摄影视角、空间布局、人物与物体数量、位置关系、比例关系、动作姿态和整体结构；不改变核心内容，不添加、不删除、不重新设计任何元素，不改变主体身份特征。
材质：人物、动物、物体与环境均采用高精度塑形黏土（plasticine clay material），柔软橡皮泥质感（soft rubbery clay texture）、手工雕刻痕迹、细腻不规则的表面纹理、微小指纹、捏塑痕迹、黏土颗粒细节和真实黏土重量感；避免光滑塑料感与普通 CG 模型感。
角色：保持原有身份特征、服饰、造型与动作姿态，同时重塑为专业定格动画角色 —— 不是简单覆盖一层黏土材质。角色要可爱一些，脸部有轻微气血感；手部偏细，不要粗壮。柔软圆润的黏土身体轮廓、轻微夸张的头身比例、笨拙但讨喜的气质；避免严重畸形与怪物化。面部保持五官识别度，重塑为具电影动画美学的黏土脸部结构：流畅轮廓、协调夸张的五官比例、立体眉毛、柔软塑形鼻子、富表现力的嘴部和圆而有神、有方向感的大眼睛。
表演：强化阿德曼式喜剧表演 —— 戏剧化、滑稽、荒诞、幽默，眉毛动作夸张但自然，眼神灵活多变，嘴部表情夸张而协调，让角色像专业黏土动画演员一样有强烈表演力，而不是恐怖怪物或丑陋变形人。整体气质：笨拙、可爱、幽默、有生命力，具有《小羊肖恩》《超级无敌掌门狗》般的荒诞喜剧魅力。
场景：保持原环境结构，转化为真实存在的手工黏土世界，具有手工制作的环境细节与电影级空间质感。
渲染：真实摄影级灯光、微距摄影质感、电影级景深、柔和自然光影、HDR、真实材质反射、全局光照（Global Illumination）、PBR 物理渲染。最终效果 = 真实摄影质感 + 阿德曼黏土动画风格 + 荒诞喜剧美学 + 电影级 3D 渲染。`,
  zine: `媒介锁定：独立杂志 / 小志（zine）风格的极简编辑海报，像手工印刷的小开本刊物内页，不是插画也不是照片。
版式优先：画面本身就是一张排版作品 —— 明确的网格、大面积留白、克制的构图、清晰的视觉层级。主体图像被处理成高对比度的粗网点半调（halftone）或单色分色印刷效果，保留可见的网点、油墨不匀与轻微套印错位。
配色：整体近似单色或双色印刷，以米白、报纸灰、墨黑为底，只用一到两个高饱和强调色（如朱红、钴蓝或荧光橙）点睛，绝不多色。
质感：粗糙再生纸底纹、油墨渗透边缘、复印机式的颗粒与噪点、轻微折痕与裁切毛边，像影印后装订起来的独立出版物。
构图：主体偏离中心，四周留出大片空白；可出现细的分隔线、方框或页码式的小元素作为排版语言，但不出现任何可读的文字内容。
整体气质：安静、克制、有编辑感与手作气息，介于当代平面设计与地下出版物之间。`,
...
const STYLE_COMPOSITION_GUIDANCE: Record<ImageStyle, string> = {
  zine: "按杂志内页排版：主体偏离中心并压在网格线上，至少留出三分之一空白；景别以中景为主，元素控制在 3 个以内，靠留白和层级说话。",
  crayon: "选择能用少量角色和大体块讲清楚的中近景；背景只保留辨认地点所需的 2–4 个手工感元素，动作轮廓必须一眼可读。",
...
const STYLE_EXCLUSIONS: Record<ImageStyle, string> = {
  zine: "不要彩色照片写实、渐变、3D 渲染、动漫线稿、水彩、厚涂油画、拥挤构图、多色配色，也不要出现任何可读文字或真实商标。",
  crayon: "不要光滑塑料感、普通 CG 模型感、日漫脸、矢量描边、写实人类皮肤、恐怖谷或怪物化造型，也不要平面卡通与二维插画质感。",
